...
const commentShoe = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
const deleteComment = asyncHandler(async (req, res) => {
  const { shoeId, commentId } = req.body;

  if (!shoeId || !commentId) {
    return res
      .status(400)
      .json({ message: "Shoe ID and comment ID are required" });
  }
  ...
  const commentIndex = shoe.comments.findIndex(
  ...
  shoe.comments.splice(commentIndex, 1);
  await shoe.save();
